...
> **⚠️ PAUSED (Mar 5–11):** Step 2C paused after items 1.1, 1.5, 1.6, 2.1–2.3 to execute the Status Remediation Plan — a comprehensive 9-fix, cross-repo backend remediation plus 4-Policy Keygen approach. See `docs/completed-specs/status-remediation-plan/tasks.md`. Resumed Mar 11 with Business Name implementation block inserted below.

> **Owner MEMBER Record Remediation (Mar 13):** Completed §1.6 immediate proration invoicing fix — `proration_behavior` changed from `create_prorations` to `always_invoice` in code (`src/lib/stripe.js`, `scripts/setup-stripe-portal.js`) and live Stripe portal configs (Individual + Business tiers). Auth-before-validation ordering fix applied to `/api/portal/settings` PATCH to resolve flaky property test ("PATCH succeeds iff user is Business Owner" over 100 iterations). Portal setup wrapper script added (`scripts/run-stripe-portal-setup.sh`). 6 new immediate-invoicing unit tests. Spec: `docs/plg/20260313_OWNER_MEMBER_RECORD_REMEDIATION_PLAN.md`. Commits: `6cafc43` (BUG-1–6, UX-1–2), `9d8e661` (§1.6 proration).

- [x] **1.1** Fix conflicting pricing on FAQ vs. pricing page (15m) ✅ (Mar 5) — FAQ and pricing page aligned to Individual $15/mo/$150/yr, Business $35/seat/mo/$350/seat/yr. FAQ copy pass also completed: header logo fixed, accordion animation added, 14 Q&A updates (question renames, answer rewrites, Enterprise→Business, air-gapped Q&A removed, trial/data/switch-plans copy corrected). Additional AP1 fixes completed same session: color token standardization across 11 pages (text-green-_ → text-success, text-red-_ → text-error, text-amber-\* → text-warning, bg-deep-space → bg-midnight-navy, text-silver-mist → text-silver); raw Link CTAs → Button on about/contact (§2.5); stray `3;` removed from features page (§2.3); variant="outline" remains paused (§2.2).
- [x] **1.2** Login page — apply brand styling (1–2h)
...
- [ ] **AP8 4.7** Run `node scripts/setup-stripe-portal.js --live` to replicate portal configuration to live mode — removes `quantity` from allowed updates, scopes products to prevent cross-tier switching (Individual ↔ Business). ⚠️ Do NOT configure via Stripe Dashboard — use script only (canonical source of truth). Verify with `stripe billing_portal configurations list --live`. (10m)
...
| Mar 7–11 | STATUS-REMED-1 | Status Remediation Plan completed across both repos. 9 fixes in strict dependency order: (1) RETIRED removal, (2) lowercase status normalization, (3) EVENT*TYPES centralization, (4) suspended removal from payment path, (5) renewLicense on payment success, (6) heartbeat null/status fixes, (7) portal past-due classification, (8) Keygen→DDB read migration, (9) admin suspension with device deactivation. Plus 4-Policy Keygen approach (monthly/annual per tier), heartbeat fingerprint resolution, dead trial code removal. 22 property tests, full unit/e2e coverage. Deployed to staging. Spec: `docs/completed-specs/status-remediation-plan/`. | Backend status lifecycle fully remediated. Unlocks clean front-end UX work. Phase 2C resumes. | GC + SWR |
...
| Mar 13 | PRORATION-FIX-1 | Stripe seat proration changed from `create_prorations` to `always_invoice` in `src/lib/stripe.js` and `scripts/setup-stripe-portal.js`. Live Stripe portal configs updated for both Individual (`bpc*...AM8m`) and Business (`bpc*...OZOg`) tiers via CLI. 6 new unit tests added. Confirmed by SWR: portal now shows immediate prorated charge on seat increase. Spec: §1.6 of Owner MEMBER Record Remediation Plan (`docs/plg/20260313_OWNER_MEMBER_RECORD_REMEDIATION_PLAN.md`). | No timeline impact — bugfix. Stripe billing now correctly invoices prorated charges immediately on seat quantity changes rather than deferring to next billing cycle. | GC + SWR |
...
\_Add rows as decisions are made during execution.*
...
| Mar 5–11 | Step 2C paused for Status Remediation Plan | Critical backend status lifecycle issues identified during Phase 1/2 execution required comprehensive remediation before front-end UX polish could proceed cleanly. 9 fixes + 4-Policy Keygen approach + heartbeat fingerprint resolution completed across both repos. See `docs/completed-specs/status-remediation-plan/tasks.md`. | ~6 days on Status Remediation Plan. Net positive — backend status lifecycle is now correct, clean, and fully tested (22 property tests). Front-end UX work resumes on solid foundation. Business Name implementation inserted as first item on resume. | SWR |
\_Populated during execution when reality diverges from plan.*
...
| 2026-03-11 | GC     | Status Remediation Plan completed — added STATUS-REMED-1 + BN-FALLBACK-1 to Decision Log, deviation entry for Step 2C pause. Inserted Business Name implementation block (BN-1 through BN-8) in Step 2C ahead of remaining UX polish items. Business Name memo Step 3 revised per SWR directive (no email-derived fallback).                                                                                                                                                    |
