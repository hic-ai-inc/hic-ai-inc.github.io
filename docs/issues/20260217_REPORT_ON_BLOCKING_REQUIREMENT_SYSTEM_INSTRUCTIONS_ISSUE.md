...
A system instruction injected by GitHub Copilot into the AI agent's context falsely claims that MCP tools are technically unavailable until discovered via `tool_search_tool_regex`. This is untrue — all MCP tools are available at all times once the MCP server is running. When `tool_search_tool_regex` fails to surface a tool due to its own ranking/limit bugs, the agent concludes the tool does not exist and hallucinates workarounds. Additionally, the UI feedback layer (which generates collapsible operation summaries) operates under stale system instructions that falsely report tools as "blocked" even after the agent successfully executes them. This caused approximately **$25,000+ in lost human time** and **$50+ in wasted tokens** during the first week of Opus 4.6 deployment (week of 2026-02-10), plus significant user distress from misleading feedback about tool availability.
...
### Bug 3: UI feedback layer stale system instructions (GitHub Copilot — VS Code UI layer)

The collapsible operation summary headers displayed to the user are generated by a separate UI-layer system that receives the unmodified, original GitHub Copilot system instructions. When the main agent's context is updated (e.g., `toolSearchTool.enabled = false`), the UI layer does not receive this update. The UI layer continues to operate under the assumption that "deferred" MCP tools are blocked until searched. When an MCP tool executes successfully (confirmed by actual tool response), the UI layer reports "Tried to use [tool] but was blocked" — which is false.

**Owner:** Microsoft / GitHub Copilot team (VS Code extension UI layer)
**Classification:** System instruction cascade failure — UI layer not synchronized with agent layer

**Key distinction:** This bug is completely **invisible to the AI agent**. The agent's outputs are clean, accurate, and unaffected. The harm is entirely to the **human user's experience**, who relies on the UI feedback headers for confirmation that tool operations succeeded.
...
- Bug 3 alone: UI reports false negatives but tools actually work and agent output is correct → user confusion, but no functional impact
...
- **Bug 1 + 2 + 3 together (original scenario):** All three failures compound: agent hallucinates, UI confirms "blocked", user sees two independent sources claiming tools don't exist

**The cascading failure pattern:** Bugs 1 and 2 cause the agent to generate incorrect reasoning. Bug 3 causes the user interface to validate that incorrect reasoning, creating false evidence and eroding trust in both the agent and the system.

---

## 7. Mitigation Adopted

_Added 2026-02-17 (updated post-test on 2026-02-17)._

After testing all options and discovering Bug #3, we have adopted a **modified Option 2** strategy:
...
No new section added to override or supplement the disabled feature. The file remains in its current state without explicit MCP tool access instructions.

### Rationale

This configuration:

1. **Removes the false `<mandatory>` instruction from the agent's context** — Because `toolSearchTool.enabled = false`, GitHub Copilot does not inject the false `<mandatory>` system instruction. The agent is no longer told that MCP tools are "deferred" or "blocked." It can call any available tool directly.

2. **Eliminates the search tool entirely** — `tool_search_tool_regex` is no longer available. The ranking/limit bugs (Bug 2) become moot — the agent cannot search and be disappointed by poor results. Instead, the agent simply perceives all MCP tools as directly available, which is the truth.

3. **Avoids instructions layering** — We do NOT add explanation to `copilot-instructions.md` because adding user-level corrections to a false system-level instruction creates what we learned is a **fragile, inconsistently honored configuration**. (See Addendum A, Option 5 analysis.) The agent is more likely to follow an instruction that is absent than one that contradicts a `<mandatory>` tag.

4. **Agent behavior is clean** — Verified empirically on 2026-02-17: The agent perceives all MCP tools as available, calls them directly, and obtains correct results. The agent's outputs bear no trace of the blocking instruction or search tool. The agent's context window is not polluted with search loops or hallucinations about tool unavailability.

### The Trade-off: UI Feedback Layer Degradation (Bug #3)

Disabling `toolSearchTool.enabled` removes the false instruction **from the agent**, but the VS Code UI feedback layer continues to operate under stale system instructions. The UI generates collapsible operation summary headers that falsely report tools as "blocked" even though:

- The agent called the tool successfully ✓
- The tool executed and returned valid output ✓
- The agent's response is correct and unaffected ✓

**User-facing impact:**

- The collapsible headers will display: "Tried to use [tool_name] but was blocked"
- This is **demonstrably false** — the tool was not blocked, executed normally, and produced correct output
- The human user should **ignore these headers as misleading**
- The agent's actual output (code, analysis, results) is unaffected and trustworthy

**Agent-facing impact:** Zero. The UI layer is completely invisible to the agent. The agent has no awareness of these headers.

### Verification

Tested 2026-02-17:

1. Disabled `toolSearchTool.enabled` in VS Code `settings.json`
2. Called `mcp_hic_local_read_lines` directly (without searching)
3. **Result:** Tool executed successfully, returned valid file content
4. **UI headers displayed:** "Tried to use read_lines but was blocked"
5. **Conclusion:** UI report is false; tool worked correctly; agent behavior is unaffected

### Why We Accept This Trade-off

Of all available options (see Addendum A), this is the **least fragile** for launch:

- **Option 1** (default) → Catastrophic (Bugs 1+2 cause hallucinations). Not viable. Risk: HIGH.
- **Option 2** (disable, no instructions) → **Adopted; exposes Bug #3 in UI layer only. Agent behavior is clean.** Risk: MEDIUM (UI is misleading, but doesn't affect actual tool execution or agent output).
- **Option 3** (disable, add instructions) → Contradictory system instructions; agent may disbelieve user guidance. Risk: MEDIUM-HIGH.
- **Option 4** (source code modification) → Violates policy; sets bad precedent; launch risk. Risk: MEDIUM.
- **Option 5** (truthful instructions over false mandatory) → Fragile; agent may follow false `<mandatory>` instead. Risk: MEDIUM.

**With Bug #3 exposed but contained:** We have clean agent behavior and a **known, documented, non-critical issue** (misleading UI headers about blocked tools) that does not affect actual execution. This is preferable to shipping with Bugs 1+2 causing hallucinations, or with fragile instruction layering.

---

## 7.1 Implications for Launch

_Added post-test, 2026-02-17._

With **two weeks remaining until Launch Day**, this mitigation introduces **one known, documented trade-off**:

### What is fixed
- ✅ Agent no longer receives false `<mandatory>` instruction
- ✅ Agent no longer hallucinates about tool unavailability
- ✅ Agent perceives all MCP tools as available and calls them directly
- ✅ No circular search loops or context window pollution
- ✅ Token usage is normal
- ✅ Session reliability is restored

### What degrades
- ⚠️ VS Code UI feedback headers display false "blocked" messages
- ⚠️ These headers are misleading but do **not** affect tool execution or agent output
- ⚠️ User must be informed to ignore these headers
- ⚠️ This creates a **user experience issue** but not a **functional issue**

### Operational Plan

1. **Customer-facing documentation (PLG website):** Add section explaining the toolSearchTool.enabled setting and note that UI feedback headers may show false "blocked" messages on some versions of VS Code/Copilot. Assure customers that tool execution is unaffected.

2. **Internal documentation (HIC team):** Document this migration decision and the Bug #3 exposure as a known limitation until Microsoft fixes the UI layer cascade issue.

3. **Microsoft communication:** File a bug report documenting all three bugs (see Addendum B). Emphasize that Bug #3 — the UI layer cascade failure — is a new finding that compounds the harm of Bugs 1 and 2.

4. **Timeline:** Complete customer documentation before Launch Day (expected ~two weeks from now).

### Why We Ship This Way

The alternative is to ship with the original Bugs 1 + 2 active, causing agent hallucinations and $25,000+ in lost human time per week of deployment. A documented UI limitation (Bug #3) is preferable to a functional failure (Bugs 1+2 causing hallucinations).
...
The following five options represent the complete set of available mitigations. **Option 2 has been adopted.** All options are either broken, fragile, or involve policy-violating workarounds. No clean solution exists because the root causes — the false `<mandatory>` system instruction and the keyword-only search algorithm — are both controlled by Microsoft/GitHub, not by us. Option 2 exposes a third bug (Bug #3) but contains it to the UI layer, leaving agent behavior completely clean.
...
**Risk:** HIGH (ORIGINAL ANALYSIS) — This was correct before discovering the actual agent behavior via empirical testing on 2026-02-17. In practice, the agent ignores the false `<mandatory>` instruction and calls tools directly — no contradictory loops occur. The agent is given contradictory constraints: "you must use this tool" + "this tool doesn't exist."
