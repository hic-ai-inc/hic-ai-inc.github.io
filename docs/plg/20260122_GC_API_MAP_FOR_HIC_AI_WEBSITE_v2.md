...
**Version:** 2.0  
**Status:** Planning  
**Related:** [PLG Technical Specification v2](./20260122_GC_PLG_TECHNICAL_SPECIFICATION_v2.md)

**v2.0 Changes:**
- Tier detection updated: OSS / INDIVIDUAL / ENTERPRISE (removed TEAM)
- Device limits reconciled: OSS=1, Individual=3, Enterprise=2/seat
- Features map updated for 3-tier structure
...
export async function POST(request) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      const tier = priceId?.includes("individual") ? "INDIVIDUAL" : "ENTERPRISE";
    ...
    case "customer.subscription.updated": {
      ...
      const newTier = priceId?.includes("individual") ? "INDIVIDUAL" : "ENTERPRISE";
