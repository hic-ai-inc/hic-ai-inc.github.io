...
**Timeline:** ~58 hours total (28h TIER 1 blockers + 30h TIER 2 critical quality)
...
## Part 1: Code Security Review (~18h)
...
### 1.2 Authentication & Authorization Review - 8h
...
**Browser Token Storage Audit:**

- [ ] Verify ID token NOT in localStorage (NextAuth handles this)
- [ ] Check browser DevTools → Application → Storage (all sites)
- [ ] Test: Log out → Verify no tokens remain in storage
- [ ] Test: Close tab → Session doesn't persist inappropriately

**Rate Limiting Verification:**

- [ ] `/api/license/trial/init` - Verify fingerprint-based rate limit (1 trial per device)
- [ ] `/api/checkout` - Verify reasonable request limits
- [ ] Test rate limit bypass via header spoofing (X-Forwarded-For)
- [ ] Verify rate limits work under API Gateway (not just local)

**Webhook Replay Attack Prevention:**

- [ ] Stripe: Verify timestamp tolerance check (5 min default)
- [ ] Keygen: Verify idempotency key handling
- [ ] Test replay of valid signature with old timestamp
- [ ] Verify webhook processing is idempotent (can safely receive duplicates)

**Pre-token Lambda Trigger Security:**

- [ ] Verify trigger only adds custom claims, doesn't modify standard claims
- [ ] Test that invalid groups don't grant elevated access
- [ ] Verify Lambda errors don't block authentication (graceful degradation)
- [ ] Check CloudWatch logs don't expose user details
...
**DynamoDB Access Pattern Security:**

- [ ] Verify GSI access patterns don't expose cross-tenant data
- [ ] Test PK/SK design prevents unauthorized record access
- [ ] Verify license lookups can't enumerate other users' licenses
- [ ] Check that customer_id isolation is enforced at query level
- [ ] Review FilterExpressions vs KeyConditions (filter is post-query!)
...
**Error Response Information Leakage Audit:**

- [ ] Authentication errors: Generic "Invalid credentials" (no user enumeration)
- [ ] Authorization errors: 403 with no role/permission details leaked
- [ ] Validation errors: No internal field names exposed
- [ ] Server errors: No stack traces, no file paths
- [ ] Test: Invalid license key → should NOT reveal if key exists vs invalid format
- [ ] Test: Invalid user lookup → should NOT reveal if email is registered
...
- [ ] Verify preflight (OPTIONS) requests handled correctly
- [ ] Check Amplify CORS configuration in amplify.yml
- [ ] Verify Access-Control-Allow-Origin is NOT wildcard (*) in production
...
- [ ] Staging and production use same IAM policies (parameterized)
- [ ] Staging and production have same security headers
- [ ] Production has stricter rate limits than staging
- [ ] No debug endpoints/features enabled in production
- [ ] Verify production Cognito callback URLs are restricted

**Pre-Audit Prerequisites:**

- [ ] RBAC implementation complete (role-based access working)
- [ ] Pre-token Lambda deployed and configured
- [ ] Client-side CI/CD auto-update packaging finalized
- [ ] Webhook DLQ monitoring operational
- [ ] All E2E tests passing in staging (establish baseline)
...
|---------|------|---------|
| 1.0.0 | Feb 2, 2026 | Initial security audit plan created (Q Developer) |
| 2.0.0 | Feb 2, 2026 | Added rate limiting, webhook replay, DynamoDB access, error leakage, pre-token Lambda, CORS, and environment parity checks (Copilot) |
