...
**Context:** Comprehensive documentation plan for the Mouse PLG website. Covers the full scope of what excellent documentation looks like, triaged into pre-launch essentials vs. post-launch builds, with the Lemon Squeezy reapplication as the gating constraint.
...
4. **The LS reviewer will check the docs.** Lemon Squeezy previously rejected citing lack of website and social media. A reviewer seeing a docs section with 75% 404 rates (15 of 20 slugs) would form the same impression: incomplete product. The docs need to be _functional and credible_ for the LS application.
...
| FAQ pricing accuracy          | Wrong ($25 vs $35, "Enterprise" vs "Business") | F                        |
...
## Triage: What Must Exist Before Lemon Squeezy

The LS application requires _functional, credible documentation._ An LS reviewer will:
...
They are **not** going to read every article or evaluate technical depth. They're checking: "Is this a real product with real documentation, or a placeholder?"

### Tier 1: Must-Have for LS Application (Pre-Launch Gate)
...
This gives the LS reviewer a documentation section with:
...
**AP 1 already recommended removing dead links.** This plan concurs. For the LS application:
...
### Tier 2: Should-Have for Launch (Post-LS, Pre-Launch)

These don't need to exist for the LS application but should exist before real users start signing up. Write these during the LS wait window or immediately after.
...
| DOC-15 | Roo Code Setup                   | `roo-code-setup`    | 30m    | Completes the client coverage                                                       |
...
5. **Offline Operation** — Mouse works fully offline with a 7-day grace period after last successful heartbeat. All editing operations are local and require no network.
...
| AP 5 (remaining, during LS wait)         | Tier 2 (DOC-11 through DOC-19) | 9 articles, ~6.5h, can be done during LS wait    |
...
### Phase 1: Pre-LS Application (Tier 1)
...
| 7        | DOC-15: Roo Code Setup                 | Complete client coverage                |
...
| **AP 8A (LS Application)**        | Tier 1 docs are a **hard dependency** for LS submission.                                                             |
...
2. **Supported clients for docs:** The dependency map includes 5 clients for E2E testing (Copilot, Cursor, Kiro, Claude Code CLI, Roo Code). Should all 5 get Tier 1 integration guides, or is Copilot + Cursor sufficient for LS, with the other 3 in Tier 2? (Recommendation: Copilot + Cursor in Tier 1, others in Tier 2.)

3. **AI Agent Crib Sheets priority:** The crib sheets (DOC-16, DOC-17) are in Tier 2, but they directly increase product value and could differentiate Mouse during the LS review. Should one (the Copilot crib sheet) be promoted to Tier 1? (Recommendation: Keep in Tier 2 — the LS reviewer won't evaluate agent prompt engineering, but real users will benefit hugely.)
