...
| 2   | Stripe live-mode OR LS approval        | Required | Need live payment credentials for production secrets                        |
...
**Depends on:** P2 complete (need CF outputs); Stripe live-mode OR LS credentials available
...
├── STRIPE_SECRET_KEY         → sk_live_... (or LS equivalent)
...
> **Note on Stripe:** If staying with Stripe, live-mode products and prices must be created FIRST (P6) because their IDs are needed for `NEXT_PUBLIC_STRIPE_PRICE_*` env vars. If migrating to LS, this is replaced by LS configuration. The webhook secret (`STRIPE_WEBHOOK_SECRET`) is generated when the webhook endpoint is created in P6.
...
#### Decision: Same App or New App?

| Option                                          | Pros                                                              | Cons                                                                                                |
| ----------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **Same Amplify app, new branch** (e.g., `main`) | Simpler setup, shared build settings, uses existing `amplify.yml` | Production and staging share app-level env vars (our scripts already handle branch-level overrides) |
| **New Amplify app**                             | Complete isolation, independent app-level config                  | More setup work, new app ID, separate `amplify.yml`                                                 |

The existing codebase uses `AWS_APP_ID` in `secrets.js` to resolve SSM paths. A new branch on the same app shares the app ID; a new app gets its own. Either works — the env var `AWS_APP_ID` can be overridden.

#### Steps

**4.1** Create the production environment:

If **same app, new branch:**
...
1. Amplify Console → New app → Host web app → GitHub → `hic-ai-inc/hic-ai-inc.github.io`
2. Branch: `main`
3. App root: `plg-website`
4. Build settings: copy from existing `amplify.yml`
...
#### Lemon Squeezy (if applicable)

**6.6** If LS is the payment provider instead of Stripe:

- Create live-mode products/variants in LS
- Configure webhook: `https://hic-ai.com/api/webhooks/lemonsqueezy`
- Update Amplify env vars with LS-specific values
- Replace Stripe secrets with LS secrets in Secrets Manager

#### P6 Verification

- [ ] Stripe live products/prices exist (or LS equivalents)
...
- [ ] Stripe/LS → `hic-ai.com/api/webhooks/stripe` — verify payload received
...
- [ ] Billing portal (Stripe Customer Portal or LS equivalent) accessible
...
| 2   | **Stripe vs. LS decision finalized?**                                                                                | Affects entire P6, all payment env vars and secrets                                                                                                                  | Before P3                             |
...
_This document is a standalone AP 4 reference. It will be integrated into the broader Launch Plan and Dependency Map schedule after SWR review._
